    ...
    try:
        for vitem in itemlist:
            ...
            if vid is not None:
                ...
                #litems.append(item)
                xli = ListItem(label=lbl, label2=lbl2, icon=thumb, thumbnail=thumb, path=vpath)
                xli.thumbnail = thumb
                xli.icon = thumb
                xli.playable = True
...
def getAPIURLS(sitename=None):
    ...
    siteapis = dict(gaytube=b+"gaytube.com/api/webmasters/search/?ordering=newest&period=alltime&thumbsize=preview&count=200&page=1&search=&tags[]=&category=",
        ...
        redtube="http://api.redtube.com/?data=redtube.Videos.searchVideos&output=json&thumbsize=medium&ordering=newest&page=1&search=&tags[]=gay&category=&period=alltime",
        ...
        youporn=b+"youporn.com/api/webmasters/search?search=&page=1&ordering=newest&tags[]=gay&category=&thumbsize=medium")
...
def index():
    ...
    for li in litems:
        ...
        allitems.append(li)
    litems = sorted(allitems, key=lambda allitems: allitems['label'])
    itemsearch = {'label': 'Search All Sites', 'path': plugin.url_for(search), 'icon': __imgsearch__, 'thumb': __imgsearch__}
    itemsearch.setdefault(itemsearch.keys()[0])
    litems.append(itemsearch)
    itemstream = {'label': 'Play Flashx.TV URL', 'path': plugin.url_for(resolver), 'icon': 'DefaultFolder.png', 'thumb': 'DefaultFolder.png'}
    itemstream.setdefault(itemstream.keys()[0])
    litems.append(itemstream)
    return litems
...
def siteroot(sitename):
    ...
    if sitename.lower() == "spankwire" and not (DOSTR8 == True or DOSTR8 == 'true'):
        ...
        item = {'label': 'XTube Categories', 'path': plugin.url_for(site, sitename='xtube', section='category', url=siteurl), 'icon': igt, 'thumb': igt}
        item.setdefault(item.keys()[0])
        litems = [item]
        litems.extend(site(sitename, 'index', siteurl))
                ...
                if img.find('flashx.tv') != -1:
                    ...
                    if len(vurl) > 0:
                        try:
                            ...
                            #item.setdefault(item.keys()[0])
                            item = ListItem(label=title, label2=link, icon=thumb, thumbnail=thumb, path=vpath)
                            item.thumbnail = thumb
                            item.icon = thumb
                            item.add_context_menu_items([('Download', 'RunPlugin("{0}")'.format(plugin.url_for(download, name=title, url=vidurl)),)])
...
def play(title, video, url):
    ...
    try:
        ...
        if url.find('flashx') != -1:
            ...
            vli.playable = True
            vli.add_context_menu_items([('Download', 'RunPlugin({0})'.format(plugin.url_for(download, name=title, url=resolved)),)])
...
def download(name, url):
    ...
    nameClean = name + '.' + ext[0:3]
    savepath = ''
    savepath = plugin.get_setting('downloadpath')
    if savepath == '':
        savepath = path.expanduser('~/Downloads/')
    ...
    try:
        ...
        xbmc.log("FAILED TO DOWNLOAD {0}".format(url))
    dparams = {}
    dparams["Title"] = name
    dparams["url"] = url
    dparams["download_path"] = savepath
    downloader.download(nameClean, dparams)
    plugin.notify(msg="Downloaded {0} to {1}".format(url, path.join(savepath, nameClean)), title="hubGAY Download")
...
def search():
    ...
    for k,v in siteurls.iteritems():
        ...
        allitems.extend(makeVideoItems(itemslist))
    litems = sorted(allitems, key=lambda allitems: allitems.label)
    return litems
